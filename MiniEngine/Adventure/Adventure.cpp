...
class Adventure : public GameCore::IGameApp
{
public:
...
std::vector<TextureRef> g_Textures;
...
void LoadTextures()
{
	...
	Utility::Printf("Loading environment maps\n");

	WIN32_FIND_DATA ffd;
	HANDLE hFind = FindFirstFile(L"Textures/*.dds", &ffd);
	...
	if (hFind != INVALID_HANDLE_VALUE) do
	{
		...
		TextureRef textureRef = TextureManager::LoadDDSFromFile(L"Textures/" + baseFile);
		if (textureRef.IsValid())
		{
			g_Textures.push_back(textureRef);
		}
	}     while (FindNextFile(hFind, &ffd) != 0);

	FindClose(hFind);

	Utility::Printf("Found %u environment map sets\n", g_Textures.size());

	Renderer::SetTexture(g_Textures[0]);
}

void Adventure::Startup(void)
{
...
void Adventure::Update(float deltaT)
{
...
void Adventure::RenderScene(void)
{
